...
RSpec.describe 'Api::V1::FamilyRules', type: :request do
  # POST /api/v1/family_rules/:id/delete(.json) 記事削除API(処理)
  # 前提条件
  #   なし
  # テストパターン
  #   未ログイン, ログイン中, APIログイン中
  describe 'POST #destroy' do
    subject { post api_v1_delete_family_rule_path(id: family_rule.id, format: :json), headers: auth_headers }
    let!(:family_rule) { FactoryBot.create(:family_rule, user: user) }
    let!(:point_record) { FactoryBot.create(:point_record, user: family_rule.user, obtained_point: Settings['family_rule_update_obtained_point']) }
