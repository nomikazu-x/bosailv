...
RSpec.describe 'Api::V1::ArticleComments', type: :request do
  # POST /api/v1/articles/:article_id/article_comments/create(.json) 記事コメント作成API(処理)
  # 前提条件
  #   なし
  # テストパターン
  ...
  describe 'POST #create' do
    ...
    let_it_be(:article_comment) { FactoryBot.attributes_for(:article_comment) }
    let_it_be(:article) { FactoryBot.create(:article) }
    let(:valid_attributes) { { article_comment: { content: article_comment[:content] } } }
    let(:invalid_attributes) { { article_comment: { content: nil } } }
    let(:current_user) { user }

    # テスト内容
    shared_examples_for 'OK' do
      it '作成される。' do
        expect { subject }.to change(ArticleComment, :count).by(1) && change(Infomation, :count).by(1)
      end
    end

    shared_examples_for 'NG' do
      it '作成されない。' do
        expect { subject }.to change(ArticleComment, :count).by(0) && change(Infomation, :count).by(0)
      end
    end
    ...
    shared_examples_for '[APIログイン中]無効なパラメータ' do
      let(:attributes) { invalid_attributes }
      it_behaves_like 'NG'
      it_behaves_like 'ToMsg', 422, false, 'alert.article_comment.create', nil
    end
    ...
    shared_examples_for '[APIログイン中]有効なパラメータ' do
      let(:attributes) { valid_attributes }
      it_behaves_like 'OK'
      it_behaves_like 'ToMsg', 200, true, nil, 'notice.article_comment.create'
    end
