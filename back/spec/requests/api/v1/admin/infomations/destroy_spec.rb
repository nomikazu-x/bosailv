...
RSpec.describe 'Api::V1::Admin::Infomations', type: :request do
  # POST /api/v1/admin/infomations/:id/delete(.json) お知らせ削除API(処理)
  # 前提条件
  #   なし
  # テストパターン
  #   未ログイン, ログイン中, APIログイン中, 管理者APIログイン中
  describe 'POST #destroy' do
    subject { post api_v1_admin_destroy_infomation_path(id: infomation.id, format: :json), headers: auth_headers }
    let_it_be(:infomation) { FactoryBot.create(:infomation) }
    ...
    context '管理者APIログイン中' do
      ...
      it_behaves_like 'OK'
      it_behaves_like 'ToMsg', 200, true, nil, 'notice.infomation.destroy'
    end
  end
end
