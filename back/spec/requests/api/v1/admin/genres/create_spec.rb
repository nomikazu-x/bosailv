...
RSpec.describe 'Api::V1::Admin::Genres', type: :request do
  # POST /api/v1/admin/genres/create(.json) ジャンル作成API(処理)
  # 前提条件
  #   なし
  # テストパターン
  ...
  describe 'POST #create' do
    subject { post api_v1_admin_create_genre_path(format: :json), params: attributes, headers: auth_headers }
    let_it_be(:genre) { FactoryBot.attributes_for(:genre) }
    ...
    shared_examples_for '[管理者APIログイン中]無効なパラメータ' do
      let(:attributes) { invalid_attributes }
      it_behaves_like 'NG'
      it_behaves_like 'ToMsg', 422, false, 'alert.genre.create', nil
    end
    ...
    shared_examples_for '[管理者APIログイン中]有効なパラメータ' do
      let(:attributes) { valid_attributes }
      it_behaves_like 'OK'
      it_behaves_like 'ToMsg', 200, true, nil, 'notice.genre.create'
    end
