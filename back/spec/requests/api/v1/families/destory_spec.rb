...
RSpec.describe 'Api::V1::Families', type: :request do
  # POST /api/v1/families/:id/delete(.json) 記事削除API(処理)
  # 前提条件
  #   なし
  # テストパターン
  #   未ログイン, ログイン中, APIログイン中
  describe 'POST #destroy' do
    subject { post api_v1_delete_family_path(id: family.id, format: :json), headers: auth_headers }
    let!(:family) { FactoryBot.create(:family, user: user) }
    ...
    context 'APIログイン中' do
      ...
      it_behaves_like 'OK'
      it_behaves_like 'ToMsg', 200, true, nil, 'notice.family.destroy'
    end
  end
end
